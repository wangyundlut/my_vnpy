...
from vnpy.trader.trade_Object import VtBarData
from config.config_api import config_api


ctaBase = config_api('ctaBase')
TICK_DB_NAME = ctaBase.get('ctaBase', 'TICK_DB_NAME')
SETTING_DB_NAME = ctaBase.get('ctaBase', 'SETTING_DB_NAME')
MINUTE_DB_NAME = ctaBase.get('ctaBase', 'MINUTE_DB_NAME')
HOUR_DB_NAME = ctaBase.get('ctaBase', 'HOUR_DB_NAME')
DAILY_DB_NAME = ctaBase.get('ctaBase', 'DAILY_DB_NAME')

globalsetting = config_api('global_setting')
mongoHost = globalsetting.get('global_setting','mongoHost')
mongoPort = globalsetting.getint('global_setting','mongoPort')
...
def loadTbCsv(fileName, dbName, symbol):
    ...
    start = time()
    print(u'开始读取CSV文件%s中的数据插入到%s的%s中'.format(fileName, dbName, symbol))
...
#----------------------------------------------------------------------
def loadMyTbCsv(fileName, dbName, symbol):
    """将TradeBlazer导出的csv格式的历史分钟数据插入到Mongo数据库中"""
    start = time()
    print(u'开始读取CSV文件: {0} 中的数据插入到: {1} 的 {2} 中'.format(fileName, dbName, symbol))
    ...
    for d in reader:
        ...
        bar.vtSymbol = symbol
        bar.symbol = symbol
        bar.open = float(d['开盘价'])
        bar.high = float(d['最高价'])
        bar.low = float(d['最低价'])
        bar.close = float(d['收盘价'])
        bar.date = datetime.strptime(d['时间'].split(' ')[0], '%Y/%m/%d').strftime('%Y-%m-%d')
        bar.time = d['时间'].split(' ')[1] + ":00"
        bar.datetime = datetime.strptime(bar.date + ' ' + bar.time, '%Y-%m-%d %H:%M:%S')
        bar.volume = d['成交量']
        bar.openInterest = d['持仓量']

        flt = {'datetime': bar.datetime}
        collection.update_one(flt, {'$set': bar.__dict__}, upsert=True)
        print(bar.date, bar.time)

    print(u'插入完毕，耗时：{0}'.format(time() - start))
...
if __name__ == '__main__':
    ...
    filename = os.path.join(Project_path, 'data', 'csv', 'futures')
    csv_list = ['au','cf','j','m','ma','ni','p','rb','sr','ta']
    for spe in csv_list:
        file_new = os.path.join(filename, spe + '.csv')
        loadMyTbCsv(file_new, 'VnTrader_1Min_Db', spe)
