...
函数类包括：
BaseData 基类，其他类基于此
TickData tick数据 createFromGate 从某个具体的交易所可以创建
BarData K线数据
Tradedata 成交数据类 createFromGate 从具体交易所可以创建 createFromOrderData 从订单数据类可以创建
OrderData 订单数据类 createFromGate 从具体交易所可以创建
PositionData 查询持仓类 createFromGate 从具体交易所可以创建
AccountData 账户数据
ErrorData 错误信息类
ContractData 合约信息类 createFromGate 从具体的交易所可以创建
LogData 日志数据

HistoryData K线时间序列
SubscribeReq 查询数据类
OrderReq 发送订单类
CancelOrderReq 取消订单的类
HistoryReq 查询历史数据时输入的代码
Singleton  ？？？？单例模式
...
class VtBarData(VtBaseData):
    ...
    def __init__(self):
        ...
        self.vtSymbol = EMPTY_STRING        # vt系统代码，本地系统代码？？？？
        ...
        self.openInterest = EMPTY_INT       # 持仓量  
        self.interval = EMPTY_STRING        # K线周期
...
class VtOrderData(VtBaseData):
    ...
    def createFromGateway(gateway,                          # type: VtGateway
                          ...
                          volume,                           # type: int
                          direction,                        # type: str
                          offset=EMPTY_STRING,              # type: str
                          tradedVolume=EMPTY_INT,           # type: int
                          status=STATUS_UNKNOWN,            # type: str
                          orderTime=EMPTY_STRING,           # type: str
                          cancelTime=EMPTY_STRING,          # type: str
                          ):                                # type: (...)->VtOrderData
        vtOrder = VtOrderData()
...
class VtHistoryData(object):
    ...
    def __init__(self):
        ...
        self.interval = EMPTY_STRING   # K线时间周期
